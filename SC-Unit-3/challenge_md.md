...
Important parts of code reviews are:
-  unified code styling, either a widely accepted in a field, or developed by a company; 
- code comments, better if in docstrings for easier reading;
- code flow, if a code is concise, it's much easier to read it and less comments are required;
- adoptability of the latest programming language stardards
The code is much more understandable and aesthetically pleasing if written in such a way. I do appreciate when code reviewers give constructive feedback on my code. 

A container, like a Docker container, helps to encapsulate dependencies, such as packages, used in building/ running applications in an independent environment, i.e. container. And, if let's say, you have different applications dependent on different versions of the same package / framework on the same computer, they can run independently. For the situation in the office, they can encapsulate those different things running on different computers in a container, and when need to share the application running on a specific computer, they can share image of the container as well.  
